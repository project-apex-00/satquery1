const DEFAULT_API_URL = window.location.port === "8000" 
...
function setupEventListeners() {
  ...
  auditToggle.addEventListener("click", (e) => {
    if (e.target === refreshAuditBtn) return;
    auditBody.classList.toggle("hidden");
    auditArrow.classList.toggle("open");
  });
  refreshAuditBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    loadAuditTrail();
  });
}
...
function renderFilePreview(file, imgEl, container, promptEl, nameEl, sizeEl) {
...
function renderSlotPreview(file, imgEl, container, promptEl, nameEl) {
  nameEl.textContent = file.name;
  const reader = new FileReader();
  reader.onload = (e) => {
    imgEl.src = e.target.result;
...
function renderAnalysisResults(data) {
  ...
  geminiAnswerEl.textContent = data.answer || "No synthesis returned.";
